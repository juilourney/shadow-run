...
<div class="game-section" id="gs-guide">
  <div class="scroll-body" style="padding:calc(var(--safe-top) + 12px) 18px 40px">
    ...
    ${section('🎭', '팀 및 특수 역할 배정', `
      ...
      ${roleRow('👑 엘리트', '번개 마일리지 2배 적립', '팀의 핵심 마일리지 기여자. 투표로 팀이 적발되면 마일리지가 50% 감소하고, 역할까지 엘리트로 들통나면 2배 능력마저 박탈됩니다.')}
      ${roleRow('⚓ 앵커', '게이지를 통째로 끌어온다', '번개에서 얻은 마일리지(버프 포함)만큼 상대팀 게이지에서 깎는 동시에 내 팀 게이지에 더합니다. 한 번에 양쪽을 움직여 강력한 역전 기회를 만듭니다.')}
    ...
    ${section('⚡', '번개와 팀 고유 스킬', `
      ...
      ${row('인증 마감 시한', '시작 시각 + 예상 완주 시간(거리×페이스) + 여유 2시간이 지나면 번개가 자동 만료됩니다.')}
      ${row('인증 실패 페널티', '마감까지 인증 없이 자동 만료되면 참가자 전원 마일리지 -50% 지급 후 참여가 해제됩니다.')}
    `)}

    ${section('🗳️', '투표 및 정체 공개', `
      ${row('일시', '주 2회 — 월요일, 목요일 18:00 ~ 22:00')}
      ${row('진행', '전체 참가자 가운데 의심스러운 1명을 지목(필수)하고, 확신이 있으면 그 사람의 역할까지 추가로 지목합니다(선택 · 애매하면 기권). 더블 역할은 2표를 행사합니다.')}
