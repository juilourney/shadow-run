...
export function guideBody() {
  return `
    ...
    ${section('🎭', '팀 및 특수 역할 배정', `
      ...
      ${roleRow('🔍 탐정', '누군가의 팀 확인 (주 3회)', '[참가자 탭]에서 아군인지 적군인지 은밀히 판별합니다.')}
      ${roleRow('🕵️ 밀정', '누군가의 역할 확인 (주 3회)', '[참가자 탭]에서 상대의 구체적인 역할(엘리트/앵커 등)을 파악합니다.')}
      <p style="font-size:11px; font-weight:700; color:#52525b; letter-spacing:.06em; text-transform:uppercase; margin:14px 0 4px;">숨은 역할 (2주차부터)</p>
      ${roleRow('🤝 러닝메이트', '함께 달리면 팀에 힘을 싣는 숨은 역할', '정체를 숨긴 역할이라 누군지 아무도 몰라요(본인도 몰라요). 러닝메이트가 낀 번개는 팀 효과가 크게 터집니다 — 소식에 "🎆 러닝메이트의 기운이 터졌다!"가 뜨면 바로 그 번개예요. 혼자보다 여럿이 함께 뛰세요!')}
    `)}
    ...
    ${section('⚡', '번개와 팀 고유 스킬', `
      ...
      ${row('인증 실패 시', '마감까지 인증하지 않으면 자동 만료되어 <b style="color:#e4e4e7;">적립이 전혀 없습니다(0km)</b>. 반드시 인증해야 마일리지·게이지에 반영돼요.')}
    `)}

    ${section('📊', '실시간 게이지와 마일리지 공개', `
      ${para('게이지 바(양 팀의 판세)는 번개가 3개 완료되면 실시간으로 열립니다. 초반(탐색 시작)에는 개별 번개 하나로 참가자의 팀이 드러나지 않도록 잠겨 있어요. 팀별 정확한 마일리지 숫자와 격차는 그와 별개로 비공개입니다.')}
    ...
    ${section('🗳️', '투표 및 정체 공개', `
      ...
      ${row('팀 공개', '최다 득표자가 <b style="color:#e4e4e7;">전체 표의 30% 이상</b>을 받으면(동점 시 제외) 팀 소속이 타임라인에 공개되고, 마일리지가 50% 감소합니다. 이 감소는 <b style="color:#e4e4e7;">번개를 3번 완주(인증)하면 해제</b>돼요 — 뛰어서 만회하세요. <b style="color:#e4e4e7;">특수 능력은 그대로 유지</b>됩니다.')}
