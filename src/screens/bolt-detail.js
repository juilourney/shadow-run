...
import { subscribe, getBolts, getPlayers, setPendingBolt, toggleBoltLock, cancelBolt, startBolt, getBoltTeamInfo, CERT_GRACE_MS } from '../store.js';
...
function showResult(km) {
  ...
  if (certAt) {
    ...
    const stale = bolt?.startAt
      && (certAt < bolt.startAt - CERT_GRACE_MS || (bolt.deadline !== Infinity && certAt > bolt.deadline + CERT_GRACE_MS));
